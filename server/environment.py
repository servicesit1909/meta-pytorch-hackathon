...
try:
    from ..models import (
        ...
        TaskDifficulty, TaskManifest, VerifyHealth,
    )
except ImportError:
...
TASK_REGISTRY: dict[str, TaskManifest] = {
    "latency_triage": TaskManifest(
        ...
        max_steps=3,
        description=(
            "A latency alert is firing on the API gateway. "
            "Acknowledge it to transition the incident to ACK state."
        ),
    ),
...
class OpSentrixEnvironment(_Base):  # type: ignore
    ...
    def step(self, action: Any, **_: Any) -> SREObservation:
        ...
        sr = max(0.0, min(1.0, raw))
        if self._success:
            sr = max(0.0, 1.0 - self._total_reward)
        elif self._total_reward + sr > 1.0:
            sr = max(0.0, 1.0 - self._total_reward)

        self._total_reward = round(min(1.0, self._total_reward + sr), 4)
    ...
    def _tool_query_metrics(self, action: QueryMetrics) -> SREObservation:
        ...
        if (self._task and self._task.id in ("root_cause_analysis", "self_healing_remediation")
                and svc == "Payment-API" and "service_identified" not in self._milestones):
            self._milestones.add("service_identified")
    ...
    def _tool_fetch_logs(self, action: FetchLogs) -> SREObservation:
        ...
        return SREObservation(
            ...
            logs=logs, reward=0.0, done=False, success=False,
        )
    ...
    def _tool_verify_health(self, action: VerifyHealth) -> SREObservation:
        ...
        _ICON = {ServiceHealth.HEALTHY: "●", ServiceHealth.DEGRADED: "◐", ServiceHealth.DOWN: "○"}
        ...
        if (self._task and self._task.id == "self_healing_remediation"
